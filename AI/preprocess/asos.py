...
class PreprocessASOS:
    def __init__(self, ASOS_PATH:str, USER_PATH:str, df_consumption:pd.DataFrame, df_asos:pd.DataFrame):
        self.ASOS_PATH = ASOS_PATH   # "../data/ASOS/seoul_day_108/"
        self.USER_PATH = USER_PATH  # '../data/private/PowerConsumption/'

    def pick_columns(self, col_names:list, df_asos:pd.DataFrame) -> pd.DataFrame:
        df_simple = df_asos[col_names]
        return df_simple
    
    def combine_consumption(self, df_origin, df_consumption:pd.DataFrame) -> pd.DataFrame:
    ...
    def add_comfort_information(self, df:pd.DataFrame, ta_column:str, rh_column:str) -> pd.DataFrame:    # ta: 기온, rh: 습도
        temp_df = df.copy()
    ...
    def train_test_split(self, dataset:pd.DataFrame, test_size=0.2, do_shuffle=False):
        df_x = dataset[['전력사용량']]
        df_y = dataset.drop(['전력사용량', '일시'], axis=1)
        return train_test_split(df_x, df_y, test_size=test_size, shuffle=do_shuffle)
...
def run() -> tuple:
    ...
    Asos_instance = PreprocessASOS(SEOUL_PATH_108, USER_PATH, df_asos, df_consumption)
    # run
    col_names =  ['일시', '평균기온(°C)', '최저기온(°C)','평균 상대습도(%)', '평균 증기압(hPa)', '평균 중하층운량(1/10)', '최저 초상온도(°C)', '1.5m 지중온도(°C)']
    ...
    df_added_comfort = Asos_instance.add_comfort_information( df_combined, ta_column="평균기온(°C)", rh_column="평균 상대습도(%)")
    
    # 안쓰는 데이터 (사용완료) drop 
    dataset = df_added_comfort.drop(['평균기온(°C)', '평균 상대습도(%)', '평균 중하층운량(1/10)'], axis=1)

    x_train, x_valid, y_train, y_valid = Asos_instance.train_test_split(dataset)
    # test
    # display(x_train.head(3), y_valid.head(3))
